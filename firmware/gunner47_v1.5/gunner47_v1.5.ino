...
void loop()
{
  ...
  #ifdef PHOTORESISTOR_PIN
  if (!ONflag && currentMillis - lastOffTime > 1000 && currentMillis - oldMillis > 1000) {
      oldLightValue = lightValue;
      oldMillis = currentMillis;
      lightValue = analogRead(PHOTORESISTOR_PIN);
      #ifdef GENERAL_DEBUG
        LOG.print("light meter works === " + String(thisTime >= NIGHT_HOURS_START && thisTime < NIGHT_HOURS_STOP) +  "data: " + String(lightValue) + "\n\n");
      #endif
      if (thisTime >= NIGHT_HOURS_START && thisTime < NIGHT_HOURS_STOP && oldLightValue - lightValue > LIGHT_DIFF_VALUE) {
        ...
        LOG.println("on->>>>");
          ...
          loadingFlag = true;
        // set timer 15min
        TimerManager::TimerRunning = (bool)atoi("1");
        TimerManager::TimerOption = (uint8_t)atoi("5");
        TimerManager::TimeToFire = millis() + 900000;
