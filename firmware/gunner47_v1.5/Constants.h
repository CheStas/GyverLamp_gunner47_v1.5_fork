...
// photoresistor
#define PHOTORESISTOR_PIN     (A0)                       // аналоговый пин фоторезистора
#define LIGHT_DIFF_VALUE (2)
...
#define AP_NAME               ("069178")                   // имя WiFi точки доступа, используется как при запросе SSID и пароля WiFi сети роутера, так и при работе в режиме ESP_MODE = 0
#define AP_PASS               ("ccJmvmFx")                  // пароль WiFi точки доступа
...
#define NTP_INTERVAL          (24UL * 60UL * 60UL * 1000UL)        // интервал синхронизации времени (было 30 минут)
...
#define LOCAL_TIMEZONE_NAME   ("EET")                       // обозначение локального часового пояса; до 5 символов; может быть использовано, если понадобится его вывести после вывода времени
#else
...
#define NIGHT_HOURS_START     (1140U)                       // 1200U начало действия "ночного времени" (в минутах от начала суток, 23:00), текущее время бегущей строкой будет выводиться с яркостью NIGHT_HOURS_BRIGHTNESS
#define NIGHT_HOURS_STOP      (1380U)                        // конец действия "ночного времени" (в минутах от начала суток, 7:59)
...
// --- РАССВЕТ -------------------------
#define DAWN_BRIGHT           (230U)                        // максимальная яркость рассвета (0-255)
#define DAWN_TIMEOUT          (15U)                          // сколько рассвет светит после времени будильника, минут
